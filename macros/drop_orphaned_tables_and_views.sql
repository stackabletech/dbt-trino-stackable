{%- macro drop_orphaned_tables_and_views(dry_run="true") -%}
    ...
    {%- if execute -%}
        ...
        {%- for node in graph.nodes.values() | selectattr(
            ...
            {%- do current_model_locations[database_name][schema_name].append(
    ...
    {%- set cleanup_query -%}
		...
		WITH models_to_drop AS (
		  ...
		  WHERE
			LOWER(table_schema) IN ({{ '"' | join(lower(current_model_locations[database].keys()), '"') | lower }})
			AND NOT (
			  FOREACH (schema IN current_model_locations.keys()) :
				LOWER(table_schema) = LOWER('{{ schema }}')
				  AND LOWER(table_name) IN ({{ '"' | join(lower(current_model_locations[database][schema]), '"') | lower }})
			  END FOREACH
			)
		)
